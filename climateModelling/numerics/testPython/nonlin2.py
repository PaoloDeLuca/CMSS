...
def plot_results(time, theta1, theta2, PE):
    plt.figure(1)
...
#plot results
plot_results(time, theta1, theta2, PE)
...
#find kinetic energy
def equation_KE(theta, m):
    KE = np.zeros(len(theta))
    f4 = 0.5 * m * np.power(theta, 2)
    return f4

#calculate KE
KE_non_lin = equation_KE(V_non_lin[:,0], m)
KE_lin = equation_KE(V_lin[:,0], m)
